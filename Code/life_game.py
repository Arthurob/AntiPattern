...
class LifeGame:
    ...
    def create_neighbours_matrix(self):
        ...
        for index, value in np.ndenumerate(self.neighbours):
            ...
            #below
            if self.dict_neighbour_active["below"] & (row_below <  rows):
            ...
            #right
            if self.dict_neighbour_active["right"] & (column_right < columns):
            ...
            if self.include_corners:
                ...
                #left lower corner
                if self.dict_neighbour_active["lower_left"] & (row_below <  rows) & (column_left >= 0):
                    self.neighbours[index] += self.matrix[row_below][column_left]
                #right upper corner
                if self.dict_neighbour_active["upper_right"] & (row_above >= 0)  & (column_right < columns):
                    self.neighbours[index] += self.matrix[row_above][column_right]
                #right lower corner
                if self.dict_neighbour_active["lower_right"] & (row_below <  rows) & (column_right < columns):
    ...
    def createGrid(self, cell_size=5, color_square=0, color_background=255,
        ...
        if self.method == "normal":
            return self.createGrid_normal(cell_size, color_square, color_background,
                   color_gridlines, show_gridlines, margins, cell_margin)
        else:
            return self.createGrid_matrix(cell_size, color_square, color_background,
                   color_gridlines, show_gridlines, margins, cell_margin)
